...
const UpdateAppointment = ({ taskID, franchiseName, onModalClose }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (action === "updateAttendance") {
      ...
      } else if (action === "unassignServiceProvider") {
        await planboardService.UnassignServiceProvider({
        ...
        notifications.show({
          title: "Success",
          message: "Service provider has been unassigned from this task.",
          color: "green",
        });
      }
  ...
  const taskHasAssignedProvider = (() => {
    if (!taskInfo) return false;
    const name = (taskInfo.serviceProviderName || "").trim();
    if (name && name !== "N/A") return true;
    const id = taskInfo.serviceProviderId;
    return (
      id &&
      String(id).toLowerCase() !== "00000000-0000-0000-0000-000000000000"
    );
  })();

  const canUnassign =
    taskHasAssignedProvider &&
    taskInfo?.taskStatus !== "Completed" &&
    taskInfo?.taskStatus !== "Cancelled";
  ...
  const actionOptions = [
    ...
    { value: "unassignServiceProvider", label: "Unassign", icon: IconUserMinus },
  ...
  // Hide Unassign unless a provider is assigned and task is not terminal
  const filteredWithUnassignRule = filteredActions.filter(
    (a) => a.value !== "unassignServiceProvider" || canUnassign
  );
  ...
  const finalFilteredActions = isStartTimeInFuture
    ? filteredWithUnassignRule.filter((action) => action.value !== "updateAttendance")
    : filteredWithUnassignRule;
  ...
  return (
    <div className={classes.container}>
      ...
      {(action === "updateNotes" || action === "cancelTask" || action === "unassignServiceProvider") && (
        <Paper className={classes.formSection}>
          <div className={classes.sectionTitle}>
            <IconNotes size={16} />
            <Text size="xs" weight={600}>
              {action === "updateNotes"
                ? "Update Notes"
                : action === "unassignServiceProvider"
                  ? "Unassign"
                  : "Cancel Task"}
            </Text>
          </div>
            ...
            title={
              <Text size="xs" weight={600}>
                {action === "updateNotes"
                  ? "Notes Information"
                  : action === "unassignServiceProvider"
                    ? "Unassign information"
                    : "Cancellation Information"}
              </Text>
            }
            color={action === "updateNotes" ? "blue" : action === "unassignServiceProvider" ? "orange" : "orange"} 
            variant="light"
            className={classes.alert}
            p="xs"
          >
            <Text size="xs">
              {action === "updateNotes"
                ...
                : action === "unassignServiceProvider"
                  ? "Optional note: this will be stored on the task. The task log will record who was assigned and that the task is now unassigned."
