...
import { localStoreService, organizationService, paymentService, profileService, clientPayerService } from "core/services";
...
export default function PaymentReport({ info, type }) {
  ...
  const [payerBillToAddress, setPayerBillToAddress] = useState('');
  const [isLoading, setIsLoading] = useState(true);
  const [currencySign, setCurrencySign] = useState('$');

  /** For billing invoices: who is printed under &quot;Bill to&quot; (payer, guarantor, or client). Wage: unchanged. */
  const getInvoiceBillToDisplay = () => {
    if (type === 'wage' || !info) {
      return {
        ...
        serviceRecipientName: null,
      };
    }
    const apiAddr = (info.billToAddress || info.BillToAddress || '').trim();
    const apiPhone = (info.billToPhone || info.BillToPhone || '').trim();
    const btt = Number(info.billToType ?? info.BillToType ?? 1);
    const displayName = (info.billToDisplayName || info.BillToDisplayName || '').trim();
    const debtorEmail = (info.debtorEmail || info.DebtorEmail || '').trim();
    const clientName = [userInfo?.firstName, userInfo?.lastName].filter(Boolean).join(' ').trim()
      || (info.clientName || info.ClientName || '').trim()
      || '';
    const clientPhone = [userInfo?.mobileNo, userInfo?.phoneNo].map((s) => (s != null ? String(s).trim() : '')).find(Boolean) || '—';

    if (btt === 2) {
      return {
        name: displayName || clientName,
        address: apiAddr || payerBillToAddress || '—',
        phone: apiPhone || '—',
        email: debtorEmail || '—',
        serviceRecipientName: clientName || null,
      };
    }
    if (btt === 3) {
      return {
    ...
    if (userInfo) {
      return {
        name: displayName || [userInfo.firstName, userInfo.lastName].filter(Boolean).join(' ').trim() || clientName,
        address: apiAddr || userInfo.addressLine1 || '—',
        phone: apiPhone || clientPhone,
        email: debtorEmail || userInfo.email || '—',
        serviceRecipientName: null,
      };
    }
    return {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (type === 'billing' && info) {
          const btt = Number(info.billToType ?? info.BillToType ?? 0);
          const bpid = info.billToPayerId || info.BillToPayerId;
          if (btt === 2 && bpid) {
            try {
              const payers = await clientPayerService.getPayers();
              const arr = Array.isArray(payers) ? payers : [];
              const p = arr.find((x) => String(x.id ?? x.Id) === String(bpid));
              if (p) {
                const parts = [p.billingAddressLine1, p.billingAddressLine2, p.billingAddressLine3]
                  .map((x) => (x != null ? String(x).trim() : ''))
                  .filter(Boolean);
                if (parts.length) setPayerBillToAddress(parts.join(', '));
              }
            } catch (e) {
              console.error("PaymentReport: could not load payer for bill-to", e);
            }
          }
        }
      } catch (error) {
  ...
  }, [info?.id, type, info?.billToType, info?.BillToType, info?.billToPayerId, info?.BillToPayerId]);
  ...
  const openAsA4 = () => {
    ...
    const btA4 = getInvoiceBillToDisplay();
    const billToCareRecipientA4 =
      type === "billing" && btA4.serviceRecipientName
        ? `<div style="margin-top:8px;font-size:12px;color:#555">Client (care recipient): ${escHtml(btA4.serviceRecipientName)}</div>`
        : "";
            ...
            <div class="billing-section">
              <div class="bill-to">
                ...
                <div class="client-details">
                  <div class="client-name">${escHtml(btA4.name || "—")}</div>
                  <div>${escHtml(btA4.address)}</div>
                  <div>${escHtml(btA4.phone)}</div>
                  <div>${escHtml(btA4.email)}</div>
                  ${billToCareRecipientA4}
                </div>
              </div>
      ...
      <Grid gutter="lg">
        ...
        <Grid.Col span={9}>
          <Stack spacing="lg">
            ...
            {/* Bill to (invoice debtor) or payroll payee */}
            <Card shadow="sm" p="md" withBorder>
            <Grid>
            <Grid.Col span={6}>
               <Title order={5} mb="md">{type === "wage" ? "To:" : "Bill to:"}</Title>
              {type === "wage" ? (
                userInfo ? (
                  <Stack spacing="xs">
                ...
                (() => {
                  const bt = getInvoiceBillToDisplay();
                  const hasLine = Boolean(
                    (bt.name && String(bt.name).trim()) ||
                    (bt.email && String(bt.email).trim() && bt.email !== "—")
                  );
                  return hasLine || bt.serviceRecipientName ? (
                    <Stack spacing="xs">
                      <Text size="lg" weight={600}>{bt.name || "—"}</Text>
                      <Text size="sm">{bt.address}</Text>
                      <Text size="sm">{bt.phone}</Text>
                      <Text size="sm">{bt.email}</Text>
                      {bt.serviceRecipientName && (
                        <Text size="xs" color="dimmed">
                          Client (care recipient): {bt.serviceRecipientName}
                        </Text>
                      )}
                    </Stack>
                  ) : (
                    <Text color="dimmed">Bill-to information not available</Text>
                  );
                })()
              )}
              </Grid.Col>
