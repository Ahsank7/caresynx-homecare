...
export const ServiceProviderContractInfo = ({ userId, organizationId, readOnly = false }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const result = await profileService.upsertContractInfo(contractInfoObj);
      notifications.show({
        withCloseButton: true,
        autoClose: 5000,
        title: "Success",
        message: result?.message || "Contract information saved successfully",
        color: "green",
        style: {
          backgroundColor: "white",
        },
  ...
  return (
    <>
      ...
      <ProfileTabPanel
        title="Contract Info"
        description="Employment or service contract dates and rate for this provider."
      >
        <form
          onSubmit={
            readOnly
