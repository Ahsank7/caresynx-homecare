...
import { AppModal, AppDrawer } from "shared/components";
...
export function Payments({ userId, type, readOnly = false }) {
  ...
  /** [uspGetBillingInfo] / [uspGetWageInfo] @pSortColumn values */
  const paymentSortColumnMap = {
    id: "Id",
    userNo: "UserNo",
    name: "ClientName",
    date: "Date",
  ...
  const fetchPayments = useCallback(async (filterOverrides = {}) => {
    const getPayments = async () => {
      ...
      const request = {
        userId: userId || null,
        userNo:
          filterOverrides.userNo !== undefined
            ? filterOverrides.userNo || null
            : userNo || null,
        date:
          filterOverrides.date !== undefined
            ? filterOverrides.date || null
            : date || null,
        transactionId:
          filterOverrides.transactionId !== undefined
            ? filterOverrides.transactionId || null
            : transactionId || null,
        franchiseId: franchise,
        sortColumn:
          paymentSortColumnMap[sortStatus.columnAccessor] ?? "Date",
        sortType: sortStatus.direction === "asc" ? "ASC" : "DESC",
    ...
    return getPayments();
  }, [userId, type, userNo, date, transactionId, pageNumber, pageSize, currentFranchiseId, franchiseLoading, sortStatus.columnAccessor, sortStatus.direction]);
  ...
  const handleFilter = async () => {
    ...
    await fetchPayments({ pageNumber: 1 });
  };
  ...
  return (
    <>
        <ProfileTabPanel
          title={type === "billing" ? "Billing Info" : "Wage Info"}
          description={
            type === "billing"
              ? "Invoices, amounts, and payment status for this client."
              : "Wage payments and payroll records for this service provider."
          }
          headerActions={
            <Group spacing="xs">
              <Button
