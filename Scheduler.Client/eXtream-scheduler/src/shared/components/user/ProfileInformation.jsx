...
const schema = zod.object({
  ...
  surName: zod.string().trim().max(50, "Sur Name must be 50 characters or less"),
  ...
  userName: zod.string(),
  phoneNo: zod.string().trim().max(15, "Phone Number must be 15 characters or less").regex(/^\+?[0-9]+$/, "Phone number should only contain numbers and optionally start with +"),
  mobileNo: zod.string().trim().max(15, "Mobile Number must be 15 characters or less").regex(/^\+?[0-9]+$/, "Mobile number should only contain numbers and optionally start with +"),
  email: zod.string().trim().email("Please enter a valid email address").max(100, "Email must be 100 characters or less"),
  ...
  notes: zod.string().max(500, "Notes must be 500 characters or less"),
  userNo: zod.string(),
...
export function ProfileInformation({ id, userType, onProfileDataUpdate, readOnly = false }) {
  const { franchiseId: currentFranchiseId } = useFranchise();
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (!isValidUserID) {
        ...
        if (!helperFunctions.isValidGUID(createdUserId)) {
          throw new Error("User was saved but the new profile ID was not returned.");
        }

        form.setFieldValue("id", createdUserId);
        navigate(`/franchises/${franchiseName}/profile/${createdUserId}/${userType}`, {
          replace: true,
        });
      } else {
  ...
  return (
    <>
      ...
      <form onSubmit={readOnly ? (e) => e.preventDefault() : form.onSubmit(handleSubmit)}>
        ...
        <ProfileTabPanel
          title="Profile"
          description="Personal details, identification, and registered address for this user."
        >
        <Paper withBorder p="md" radius="md">
          <Title order={5} size="h4" mb="sm">
            Basic Information
          </Title>
