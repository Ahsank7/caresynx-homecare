import { Button, TextInput, Loader, Select, LoadingOverlay } from "@mantine/core";
...
const schema = zod.object({
  ...
  // Attach the error to `expiryYear` so a valid month isn't shown as invalid.
  path: ["expiryYear"]
});

export const CardInfo = ({ userId, organizationId, userType, readOnly = false }) => {
  ...
  const form = useForm({
    ...
    // Prevent stale validation errors when the user selects month/year in any order.
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const result = await accountService.upsertUserCardInfo(userCardInfoObj);
  ...
  return (
    <>
      ...
      <ProfileTabPanel
        ...
        description={
          currentUserType === UserType.Clients
            ? "Card on file for billing. Clients may be charged a small validation amount when saving."
            : "Payment card details for this user."
        }
      >
      <form
        onSubmit={
