...
export function Leave({ userId, organizationId, userType, readOnly = false }) {
  ...
  const handleDelete = async () => {
    try {
      const result = await leaveService.deleteLeaveItem(currentRowId);
      notifications.show({
        title: "Success",
        message: result?.message || `${leaveLabel} removed successfully`,
        color: "green",
      });
      fetchLeave();
    } catch (error) {
  ...
  return (
    <>
      ...
      <ProfileTabPanel
        ...
        description={
          isClient
            ? "Record when this client is off service or unavailable."
            : "Manage leave periods and availability for this service provider."
        }
        headerActions={
          !readOnly ? (
