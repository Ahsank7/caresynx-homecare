...
import SimpleCaptcha, { createCaptchaChallenge, isCaptchaValid } from "shared/components/SimpleCaptcha";
import {
  DashboardMockup,
  HowItWorks,
  StatsRings,
...
const CareSyncHomeCarePage = () => {
  ...
  const [contactCaptcha, setContactCaptcha] = useState(() => createCaptchaChallenge());
  const [demoCaptcha, setDemoCaptcha] = useState(() => createCaptchaChallenge());

  const contactForm = useForm({
    initialValues: { name: "", email: "", phone: "", message: "", captchaAnswer: "" },
    validate: {
      ...
      captchaAnswer: (value) => (isCaptchaValid(value, contactCaptcha.answer) ? null : "Please solve the security check"),
    },
  });

  const demoForm = useForm({
    initialValues: { name: "", email: "", phone: "", company: "", preferredDate: "", message: "", captchaAnswer: "" },
    validate: {
      name: (value) => (value.length < 2 ? "Name must be at least 2 characters" : null),
      email: (value) => (/^\S+@\S+$/.test(value) ? null : "Invalid email"),
      phone: (value) => (value.length > 0 && value.length < 10 ? "Invalid phone number" : null),
      company: (value) => (value.length < 2 ? "Company name must be at least 2 characters" : null),
      captchaAnswer: (value) => (isCaptchaValid(value, demoCaptcha.answer) ? null : "Please solve the security check"),
    },
  });

  const refreshContactCaptcha = () => {
    setContactCaptcha(createCaptchaChallenge());
    contactForm.setFieldValue("captchaAnswer", "");
    contactForm.clearFieldError("captchaAnswer");
  };

  const refreshDemoCaptcha = () => {
    setDemoCaptcha(createCaptchaChallenge());
    demoForm.setFieldValue("captchaAnswer", "");
    demoForm.clearFieldError("captchaAnswer");
  };

  const handleContactSubmit = async (values) => {
    try {
      setIsSubmittingContact(true);
      const { captchaAnswer, ...contactRequest } = values;
      const response = await landingService.submitContactRequest(contactRequest);
      if (response?.success) {
  ...
  const handleDemoSubmit = async (values) => {
    try {
      ...
      const { captchaAnswer, ...demoRequest } = values;
      const response = await landingService.requestDemo(demoRequest);
      if (response?.success) {
              ...
              <Paper shadow="xl" p="xl" radius={20} style={{ background: "white", border: "1px solid rgba(99, 102, 241, 0.08)" }}>
                ...
                <form onSubmit={demoForm.onSubmit(handleDemoSubmit)}>
                  <Stack spacing="md">
                    ...
                    <SimpleCaptcha
                      challenge={demoCaptcha}
                      inputProps={demoForm.getInputProps("captchaAnswer")}
                      onRefresh={refreshDemoCaptcha}
                      mt={0}
                    />
                    <Button
                      className="landing-btn-lift"
              ...
              <Paper shadow="xl" p="xl" radius={20} style={{ background: "white", border: "1px solid rgba(99, 102, 241, 0.08)" }}>
                ...
                <form onSubmit={contactForm.onSubmit(handleContactSubmit)}>
                  <Stack spacing="md">
                    ...
                    <SimpleCaptcha
                      challenge={contactCaptcha}
                      inputProps={contactForm.getInputProps("captchaAnswer")}
                      onRefresh={refreshContactCaptcha}
                      mt={0}
                    />
                    <Button
                      className="landing-btn-lift"
