...
const RolePermissionManagement = ({ organizationId }) => {
  ...
  const loadRoles = async () => {
    try {
      ...
      const response = await roleService.getAvailableRolesForAssignment(organizationId);
      ...
      if (rolesData.length > 0) {
        ...
        notifications.show({
          title: 'Info',
          message: 'No roles are available for you to manage at your permission level, or none exist for this organization.',
          color: 'blue'
        });
      }
    } catch (error) {
