...
--exec [Staff].[uspGetAllStaffs]    @pStaffId =null,	@pUserId =null,	@pEmail =null,	@pPhoneNumber =null,	@pJoiningDate =null,	@pLastName =null,	@pFirstName =null,	@pGenderId =null,	@pEthnicityId =null,	@pStatusId =null,	@pSortColumn  =null,	@pSortType =null ,	@PageNumber  =1,	@pPageSize  =10

CREATE PROCEDURE [Staff].[uspGetAllStaffs] 	
...
BEGIN
	...
	Into  #Results
	From  [dbo].[tblUser] U
	JOIN [dbo].tblStaff C on C.UserId=U.Id
	JOIN [dbo].tblLookupItems LI on LookupType='UserStatus' and  LI.Id=U.[Status]
	-- LEFT JOIN to get staff member's role for hierarchy filtering
    ...
    WHERE 1=1
	 ...
	 and (
		@vCurrentUserRoleLevel IS NULL  -- No filtering if current user has no role
