...
CREATE PROCEDURE [dbo].[uspGetPlanboardTasks] 
...
select st.Id as TaskId   
   ...
   ,CASE WHEN st.ServiceProviderId IS NULL THEN N'Unassigned' ELSE su.FirstName+' '+ISNULL(su.SurName,'')+' '+su.LastName END as ServiceProviderName   
   ,CASE WHEN st.ServiceProviderId IS NULL THEN N'N/A' ELSE IsNull(su.Email,'N/A') END as ServiceProviderEmail  
   ,CASE WHEN st.ServiceProviderId IS NULL THEN N'N/A' ELSE IsNull(su.PhoneNo,'N/A') END as ServiceProviderPhone  
   ,CASE WHEN st.ServiceProviderId IS NULL THEN N'N/A' ELSE IsNull(su.MobileNo,'N/A') END as ServiceProviderMobile  
   --,IsNull(st.,'N/A') as ServiceProviderAddress  
   ,st.IsConfirmed  
 ...
 and ((ISNULL(@pClientUserNo,'') = '') OR (U.UserNo = @pClientUserNo))  
 ...
 and ((@pTaskStatusIds is Null) OR (st.[Status] IN (SELECT value FROM STRING_SPLIT(@pTaskStatusIds, ','))))   
...
CASE WHEN @pSortColumn = 'ServiceType' AND @pSortType ='ASC' THEN ServiceType END ,  
CASE WHEN @pSortColumn = 'ServiceType' AND @pSortType ='DESC' THEN ServiceType END DESC,
CASE WHEN @pSortColumn = 'ServiceName' AND @pSortType ='ASC' THEN ServiceName END ,  
CASE WHEN @pSortColumn = 'ServiceName' AND @pSortType ='DESC' THEN ServiceName END DESC
